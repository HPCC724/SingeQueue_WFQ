...
import ch.ethz.systems.netbench.xpt.AdaptiveWFQTCP.AdpWFQTCPSocket;
import ch.ethz.systems.netbench.xpt.tcpbase.FullExtTcpPacket;

public class AdpWFQDCTCPTransportLayer extends TransportLayer {
    public AdpWFQDCTCPTransportLayer(int identifier){
        super(identifier);
    }

    @Override
    protected Socket createSocket(long flowId, int destinationId, long flowSizeByte) {
        return new AdpWFQDCTCPSocket(this, flowId, this.identifier, destinationId, flowSizeByte,0,-1);
    }

    protected Socket createSocket(long flowId, int destinationId, long flowSizeByte,float weight,int flowset_num) {
        return new AdpWFQDCTCPSocket(this, flowId, this.identifier, destinationId, flowSizeByte,weight,flowset_num);
    }
