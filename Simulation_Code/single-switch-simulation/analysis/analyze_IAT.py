...
class Flow:
    ...
    def calcu_pp(self,L):     #第一种方式：第一个和第二个各自除以第一个IAT的一半
        for i in range(0,len(self.IATs)):
            self.rates.append(self.sizes[i]/self.IATs[i])
...
def analyze_IAT:
    ...
    for flow in flows:
        if len(flow.IATs) > 1:
            ...
            flow.calcu_pp()
    sorted_flows =  sorted(flows,key=lambda x:x.sent_bytes,reverse=True)
    print('Writing to result file pktIAT.statistics...')
    with open(analysis_folder_path+"/pkt_IAT.statistics"+L+".csv","w",newline = '') as csvfile:
